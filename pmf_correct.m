...
randn('state',0); 
  ...
  load moviedata_s3_new % Triplets: {user_id, movie_id, rating} 
  mean_rating = mean(train_vec(:,3)); 
 
  pairs_tr = length(train_vec); % training data 
  pairs_pr = length(probe_vec); % validation data 

  numbatches= batches; % Number of batches  
  num_m = movie_count;  % Number of movies 
  num_p = user_count;  % Number of users 
...
for epoch = epoch:maxepoch
  ...
  save pmf_30_s3 err_train err_valid
